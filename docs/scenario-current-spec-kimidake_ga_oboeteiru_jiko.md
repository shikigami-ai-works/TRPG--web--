...
5. `docs/NEXT_CHAT_HANDOFF_2026-06-02_final_choice_ux_gate.md` と `docs/NEXT_CHAT_HANDOFF_2026-06-02_nontrue_scene7_ux_audit.md`: Scene 7 final-choice gate、true / normal / good / lost ending の最新 CDP 監査、検証結果、再開手順。
6. `docs/NEXT_CHAT_HANDOFF_2026-06-01_scene7_ending_verification.md`、`docs/NEXT_CHAT_HANDOFF_2026-06-01_browser_scene7_wedding_rings.md`、`docs/NEXT_CHAT_HANDOFF_2026-06-01_post_scene7_publish.md`: app 側確認、指輪 UI 確認、publish 状態の履歴。
7. `docs/NEXT_CHAT_HANDOFF_2026-05-30_scene7_return_fire.md`、`docs/NEXT_CHAT_HANDOFF_2026-05-30.md`、`docs/NEXT_CHAT_HANDOFF_2026-05-30_ritual_order.md`、`docs/scenario-writing-*.md`: 履歴として参照する。現行仕様と食い違う場合は、このファイル、YAML、現行本文、最新 handoff を優先する。
...
- 直近検証: 2026-06-02 に Chrome headless/CDP fallback で `scene_007_return_fire` 終盤を確認済み。true route は `return_with_akari` / `true` / `双つ灯の生還`、非 true route は `return_without_akari` / `normal` / `空席に残る灯`、`stay_with_akari` / `good` / `境界に灯るふたり`、`boundary_collapse` / `lost` / `境界の狭間に揺れる` に到達した。final choice は `burn_keepsakes_as_farewell` で `regret_resolved` が立つまで disabled のままであることを確認済み。コマンド検証は `npm run validate:scenarios` が 1 pack / 0 errors / 0 warnings、`npm run test` が 19/19 tests、`npm run typecheck`、`npm run lint`、`npm run build`、`git diff --check` が pass。最新の repo 保存点は `f8161ff Document Kimidake Scene 7 UX audit`。
...
handoff は作業時点の証跡と再開手順として読む。古い handoff に書かれた Git 状態、未追跡ファイル、次に実行する作業、push 済み commit は、その handoff 作成時点の情報であり、現在の指示としてそのまま再実行しない。

`docs/NEXT_CHAT_HANDOFF_2026-06-02_final_choice_ux_gate.md` は `af251e1` 時点の final-choice gate 修正と true route CDP 証跡を残した履歴である。`docs/NEXT_CHAT_HANDOFF_2026-06-02_nontrue_scene7_ux_audit.md` は non-true ending CDP 監査結果を残した履歴であり、その内容は `f8161ff Document Kimidake Scene 7 UX audit` で repo に保存済みである。

以後の再開時は、handoff の本文より先にこの current-spec、現在の `git status`、最新 commit、YAML、必要な最新証跡を確認する。
...
`docs/scenario-body-kimidake_ga_oboeteiru_jiko/*.md` の本文は、今後も改稿され得る。ここで凍結しているのは終盤順序、分岐条件、アプリ実装契約、参照優先順位であり、本文表現そのものを最終稿として固定する意味ではない。

本文と YAML / UI 文言を照合するときは、本文だけを根拠にアクションID、条件、分岐順、プレイヤー可視ラベルを過剰同期しない。本文が変わる可能性を前提に、固定契約は current-spec と YAML で確認し、本文はプレイヤー体験や描写意図の参照として扱う。

## 今後の作業で守ること

- 本文を直すときは、`01_overview.md`、該当 scene 本文、`scenario.yaml`、該当 YAML、必要ならテストを同時に確認する。
- 終盤仕様を変える場合は、このファイルも更新する。
- ending 解決順を確認するときは、`endings.yaml` だけでなく `scenario.yaml` の `ending_resolution_order` を必ず見る。
- 実装上の判断や妥協点が仕様書にない場合は、`docs/implementation-notes.md` に残す。
- Codex に自動で進めさせる工程、しき確認で止める工程、スキル/エージェント利用、検証ゲートは `docs/codex-autonomous-workflow.md` に従う。
- `.runtime/` の CDP helper はローカル監査証跡として扱う。tracked tooling に昇格する場合は、絶対パス、Chrome/Node パス、ポート、出力先、対象ルートを設定化し、`scripts/` 配下の小さな再利用可能 runner と `npm` script として別タスクで設計する。
