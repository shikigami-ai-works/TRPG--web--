...
1. `docs/scenario-body-kimidake_ga_oboeteiru_jiko/08_scene6.md` を作成するか、既存構成に合わせて四方の部屋本文を追加する。
2. scene6本文では、必ず以下の順序を守る。
   - 儀式を乱す。
   - 親族夫婦と衝突する。
...
3. 本文作成後に `npm run validate:scenarios` と `npm run test` を通す。
...
終盤順序は「儀式を乱す→親族夫婦戦→まだ指輪は取らない→真壁戦→無人の儀式場に二人→二人分の儀式再現を思いつく→ペア媒介として指輪を思いつく→遺体から指輪取得→儀式再現」で固定です。
次は 08_scene6.md の本文を、この順序と教団人数の理由付けに沿って作成してください。
```
